...
export default function Table(props) {
    const [state, setState] = useState([{ id: 1, name: "mohammad", last_name: "ahmadi", email: "mohammad@gmail.com" },
    ...
    let filtersearch = state.filter(item => item.name.toLowerCase().startsWith(search.toLowerCase()));
    return (
        <div>
            ...
            <div className="table-phone">
                <table className="table table-hover">
                    ...
                    {filtersearch.map(item => (
                        <RowTable key={state.id} item={item} />
                    ))}
                </table>
            ...
            <button className="btn btn-primary" onclick={show_form}>Add</button>
            <button className="btn btn-secondary" onclick={hide_form}>Cancel</button>
        </div>
    )
}
